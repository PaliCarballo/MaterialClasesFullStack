...
VALIDACIONES EN LARAVEL en el archivo Controller
  ...
  Para persistir los datos dentro del form, en el value del input pongo ->
    ...
    value="{{ old('name', $producto->name) }}" // persiste el campo y coloca un dato por defecto
...
public function update($id, Request $request){
  //dd($request->all());
  //dd($request->input('title')); // es lo mismo que $request->title

  // me traigo los datos por ::find
...
PASOS PARA GUARDAR UNA IMG

//preparo el form
1) en el form -> multipart-entype
  input -> type = file , name = "nombreDeMiImg"

// subo el archivo
  2) en el Controller
    publuc function 

// SI ES OPCIONAL SUBIR UNA IMG le digo que si subo un archivo lo guardo
if($request->file('poster')) {
 
    // el archivoque subi lo guardo en el filesystemde laravel
    $rutaDelArchivo = $request->file('poster')->store('public');
    // modifica el nombre de la ruta y dejo solo el nombre del archivo 
    $nombreArchivo = basename($rutaDelArchivo);
    //SOLO SI QUIERO UNA IMG POR DEFAULT agregar en la migracion una img por default y dsp hacer php artisan rolleback para actualizarla
    ...
    //tengo q validar las img, solo jpg png gif
